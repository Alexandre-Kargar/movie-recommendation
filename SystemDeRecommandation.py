...
while rst :
    ...
    if choice.lower() == "film":
        ...
        nom = nom.capitalize()
        if nom in df_movies['title'].values:
            print("\nVoici quelques recommandations pour", nom)
            print(get_recommendations(nom).to_string())
        else :
            print("\nDésolé, ce film n'est pas dans notre base de données.")
